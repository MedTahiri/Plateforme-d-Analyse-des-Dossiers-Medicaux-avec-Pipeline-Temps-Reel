...
import {
    ...
    CheckCircle,
    XCircle,
    Stethoscope, Bell, AlertTriangle
} from "lucide-react";
...
export default function Medecin() {
    ...
    useEffect(() => {
        const medecinStr = sessionStorage.getItem('loggedInUser'); // <-- ici sessionStorage
        if (!medecinStr) {
            setError("Aucun medecin connecté !");
            return;
        }
        const medecin = JSON.parse(medecinStr);
        console.log(medecin)
        console.log(medecin?.role === "ROLE_MEDECIN")
        if (medecin?.role === "ROLE_MEDECIN") {

            const medecinId = medecin?.user?.id || medecin?.user?.id;
            console.log(medecinId)
            if (!medecinId) {
                setError("ID du medecin non trouvé !");
                return;
            }
            let client = null;
            ...
            client.connect({}, (frame) => {
                ...
                // 🔁 Remplace {medecinId} dynamiquement
                client.subscribe(`/topic/alertes/medcin/${medecinId}`, (message) => {
                        ...
                        {currentMedecin && (
                            <div className="flex items-center space-x-3 px-4 py-2 bg-white rounded-lg shadow-sm border">
                                <Avatar className="h-8 w-8 ring-2 ring-blue-100">
                                    <AvatarFallback
                                        className="bg-gradient-to-r from-blue-500 to-blue-600 text-white font-semibold text-sm">
                                    ...
                                    <Table>
                                        <TableHeader>
                                            <TableRow className="border-b border-gray-100">
                                                <TableHead
                                                    className="py-4 text-gray-700 font-semibold">Patient</TableHead>
                                                <TableHead className="py-4 text-gray-700 font-semibold">Date &
                                                    Heure</TableHead>
                                                <TableHead
                                                    className="py-4 text-gray-700 font-semibold">Statut</TableHead>
                                                <TableHead
                                                    className="text-right py-4 text-gray-700 font-semibold">Actions</TableHead>
                                            </TableRow>
                                        </TableHeader>
                                        <TableBody>
                                            {rendezVous.length > 0 ? (
                                                        ...
                                                        <TableCell className="py-4">
                                                            <div className="flex items-center space-x-3">
                                                                ...
                                                                <div>
                                                                    ...
                                                                    <p className="text-sm text-gray-500">Patient
                                                                        #{item?.patient?.id}</p>
                                                                </div>
                                                            </div>
                                                        </TableCell>
                                                                ...
                                                                {item?.status !== 'annulé' && item?.status !== 'terminé' && (
                                                                    ...
                                                                    >
                                                                        {loadingActions[`annuler_${item.id}`] ? (
                                                                            <div
                                                                                className="animate-spin h-4 w-4 border-2 border-red-500 border-t-transparent rounded-full"></div>
                                                                        ) : (
                                                                ...
                                                                {item?.status !== 'annulé' && item?.status !== 'terminé' && (
                                                                    ...
                                                                    >
                                                                        {loadingActions[`terminer_${item.id}`] ? (
                                                                            <div
                                                                                className="animate-spin h-4 w-4 border-2 border-green-500 border-t-transparent rounded-full"></div>
                                                                        ) : (
